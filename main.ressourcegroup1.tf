...
resource "azurerm_log_analytics_workspace" "my-log-analytics-workspace" {
...
resource "azurerm_monitor_diagnostic_setting" "vmss-diagnostics" {
  name                 = "vmss-diagnostics"
  target_resource_id    = azurerm_windows_virtual_machine_scale_set.vmss_1.id 
  log_analytics_workspace_id = azurerm_log_analytics_workspace.my-log-analytics-workspace.id
...
resource "azurerm_automation_runbook" "Azurerunbook" {
  name                    = "Azurerunbook"
  location                = azurerm_resource_group.rg.location
  resource_group_name     = azurerm_resource_group.rg.name
...
runbook_type            = "PowerShellWorkflow"
  ...
  content = <<EOF
    Write-Host "Hello from Azure Automation!"
  EOF
}


resource "azurerm_automation_schedule" "automation-schedule" {
  name                    = "my-automation-schedule"
  resource_group_name     = azurerm_resource_group.rg.name
  automation_account_name = azurerm_automation_account.rg.name
  frequency               = "Week"
  interval                = 1
